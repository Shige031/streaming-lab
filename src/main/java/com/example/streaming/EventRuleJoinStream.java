...
public class EventRuleJoinStream {
    ...
    public void start() throws InterruptedException {
        ...
        props.put(StreamsConfig.APPLICATION_ID_CONFIG, "streaming-lab-event-rule-join-stream");
        ...
        GlobalKTable<String, Rule> rulesTable = builder.globalTable(
                ...
                Materialized.as("rules-global-store-for-join")
        );
        ...
        KStream<String, MatchedEvent> matchedEvents = events.join(
                ...
                // Event から GlobalKTable を引くための key を作る
                (eventKey, event) -> "rule_" + event.getEventName(),

                // Event と Rule が見つかったときに MatchedEvent を作る
                MatchedEvent::new
        );
        ...
        matchedEvents
                ...
                .peek((key, matched) -> {
                    ...
                    System.out.printf(
                            "[event-rule-join] MATCH eventId=%s, userId=%s, eventName=%s, ruleId=%s%n",
                            event.getEventId(),
                            event.getUserId(),
                            event.getEventName(),
                            rule.getRuleId()
        ...
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            System.out.println("[event-rule-join] shutting down...");
            streams.close();
            latch.countDown();
        }));
        ...
        System.out.println("[event-rule-join] started. Waiting for events and rules...");
